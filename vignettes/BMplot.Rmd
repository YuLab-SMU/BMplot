...
Base modification can produce many information including not only depth value and methylation value but also IPO value. IPO and IPD ratio are important indicators in single-molecule real-time sequencing. We can distinguish modified bases with it[@flusberg2010direct]. Visualizing IPO and IPO ratio in different motifs is a worthy attempt. 

The following data is simulated from GSE62206 by assigning random IPO value and IPO ratio value to each base in GSE62206. Since the `BSseq` object can only store data of methylation(X) and read depth(N), we create a `bmData` object to hold any information of base modification.
```{r fig.cap="Visualize IPO and IPO ration on different motifs",fig.align='center', fig.height=7, fig.width=12}
